...
CARDS: dict[str, dict[str, Any]] = {
    "triage_lightgbm": {
        ...
        "training_data": {
            "source": "Kaggle Triagegeist pipeline (publicly published): 80,000 SYNTHETIC ED encounters",
            "label": "ESI level as generated by the synthetic encounter pipeline",
            "demographics": "Synthetic adult and pediatric mix; not drawn from a real patient population, so demographic figures describe the generator, not a cohort",
        },
        "data_provenance": {
            "origin": "Kaggle Triagegeist pipeline: publicly published SYNTHETIC ED triage encounters. No real patient records are used to train this ensemble.",
            "lineage": "Synthetic encounter generation (upstream) -> Triagegeist feature engineering -> Solace 5-fold ensemble training.",
            "consent_basis": "Not applicable: the training set contains no real patient data, so there is no PHI and no HIPAA authorization question.",
            "time_range": "Synthetic encounters; no real-world time range applies.",
            "known_gaps": (
                "Not validated on real patient data. Distribution is the generator's, not any hospital's. "
                "US ESI conventions only; thin high-acuity peds (<2y); psychiatric-only encounters under-represented."
            ),
        },
        "risk_tier": "tier_1_high",
        # Out-of-fold metrics read from the shipped backend/models/artifacts.pkl.
        # They are the SYNTHETIC-data ceiling and must never be quoted as clinical
        # performance: a near-perfect QWK means the model recovered the generator's
        # rules, not that it triages real patients this well. Published real-data ESI
        # models sit around QWK 0.65-0.85. See docs/clinical-performance-brief.md.
        "performance": {
            "oof_quadratic_weighted_kappa_synthetic": 0.9999,
            "oof_accuracy_synthetic": 0.9998,
            "expected_real_world_qwk_range": "0.65-0.85 (published ESI benchmarks, unverified for this model)",
            "caveat": "Synthetic-data ceiling. No real-patient validation has been performed.",
        },
        ...
        "synthetic_data_caveat": (
            "This ensemble trains ENTIRELY on synthetic data: 80,000 generated ED "
            "encounters from the public Kaggle Triagegeist pipeline. No real patient "
            "records are involved. The near-perfect out-of-fold metrics are the "
            "synthetic-data ceiling, not evidence of clinical accuracy. A retrospective "
            "study against real ED registry data is required before any production use."
        ),
        "limitations": [
            "Trained only on synthetic encounters; real-patient performance is unmeasured",
            "Out-of-fold metrics reflect the data generator, not clinical accuracy",
            "Follows US ESI conventions; transfer to non-US triage systems unverified",
